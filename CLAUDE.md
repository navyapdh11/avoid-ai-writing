...
- Keep the README pattern count table (currently "35 Patterns Detected") in sync with actual SKILL.md categories
...
1. **Guardrails check** — ignores embedded instructions, handles non-English/code-heavy/short inputs
2. **Context profile detection** — auto-detects or accepts a profile hint (linkedin, blog, technical-blog, investor-email, docs, casual, creative) that adjusts rule strictness via the tolerance matrix
3. **Pattern matching** — 35 categories across content, language, structure, communication, and meta patterns
4. **Vocabulary flagging** — 3-tier system: Tier 1 (always flag), Tier 2 (flag in clusters), Tier 3 (flag at high density)
5. **Severity classification** — P0 (credibility killers), P1 (obvious AI smell), P2 (stylistic polish)
6. **Output** — rewrite mode: 4 sections (with early exit for clean text) including a conditional second-pass audit; detect mode: 2 sections with problem vs. judgment-call assessment
...
- Self-reference escape hatch has scope limitation: only applies to quoted examples under 100 words
